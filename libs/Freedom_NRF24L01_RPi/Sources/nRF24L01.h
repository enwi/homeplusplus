...
void SPI_WriteReadBuffer(const uint8_t *bufOut, uint8_t *bufIn, uint8_t bufSize);
void SPI_WriteBuffer(const uint8_t *bufOut, uint8_t bufSize);
...
void RF_WriteRegisterData(uint8_t reg, const uint8_t *buf, uint8_t bufSize);
...
#define CSN		10
#define SCKL	13
//#define MOSI	11 Predefined
//#define MISO	12
#define IRQ		2

//Settings for SPI
#define SPI_SPEED		4000000 //4MHz SPI speed, for 2Mb/s datarate
#define SPI_SETTINGS	SPISettings( SPI_SPEED, MSBFIRST, SPI_MODE0)

/* Macros to hide low level functionality */
#define RF_WAIT_US(x)  delayMicroseconds(x)  /* wait for the given number of micro-seconds */
#define RF_WAIT_MS(x)  delay(x)  /* wait for the given number of milli-seconds */
#define RF_CE_LOW()    digitalWrite(CE, 0)      /* put CE LOW */
#define RF_CE_HIGH()   digitalWrite(CE, 1)      /* put CE HIGH */
#define RF_CSN_LOW()   digitalWrite(CSN, 0)     /* put CSN LOW */
#define RF_CSN_HIGH()  digitalWrite(CSN, 1)     /* put CSN HIGH */
