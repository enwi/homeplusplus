...
// Taken and adapted from https://github.com/hobi4ek/double-axis-chart
...
export class DoubleAxisChartComponent extends BaseChartComponent implements
    OnInit, OnChanges {
  ...
  @Input() showSecondaryRefLines = false;
  @Input() secondaryReferenceLines: any;
  @Input() showSecondaryRefLabels = true;
  @Input() xScaleMin: any;
  @Input() xScaleMax: any;
  ...
  @Input('results')
  chartsData: any;  // parent class clear custom properties "secondAxis"
  ...
  update(): void {
    ...
    if (this.lineChart1.length) {
      ...
      this.ySecondScale =
          this.getYScaleLine(this.yDomainLine1, this.dims.height);
    }
  ...
  setColors(): void {
    ...
    this.colors = new ColorHelper(
        this.scheme, this.schemeType, domain, this.customColors);
  ...
  updateTimeline(): void {
    if (this.timeline === true) {
      ...
      this.timelineXScale =
          this.getXScale(this.timelineXDomain, this.timelineWidth);
      ...
      this.timelineTransform =
          `translate(${this.dims.xOffset}, ${- this.margin[2]})`;
    }
  }
}
